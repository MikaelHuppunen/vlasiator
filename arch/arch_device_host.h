...
/* Parallel reduce driver function - specialization for 2D case */
   ...
   inline static void parallel_reduce_driver(const uint (&blockDimensions)[2], const uint (&limitDimensions)[NDim], const uint* (&limits)[2], Lambda loop_body, T *sum, const uint n_redu_dynamic) {
      uint indices[NDim];
