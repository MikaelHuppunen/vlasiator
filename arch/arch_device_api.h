...
/* Parallel reduce interface function - specialization for 1 reduction variable */
   template <reduce_op Op, uint NDim, typename Lambda, typename T>
   inline static void parallel_reduce(const uint (&blockDimensions)[2], const uint (&limitDimensions)[NDim], const uint* (&limits)[NDim], Lambda loop_body, T &sum, bool dataOnDevice = false) {
      constexpr uint NReductions = 1;
      arch::parallel_reduce_driver<Op, NReductions, NDim>(blockDimensions, limitDimensions, limits, loop_body, &sum[0], NReductions, dataOnDevice);
   }
